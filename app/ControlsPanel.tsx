...
import { Building, Params, LwSide } from "./types";
...
export default function ControlsPanel({ building, setBuilding, params, setParams, setRefreshKey }: Props) {
	...
	const overlay = params?.colorOverlay ?? (defaultParams as any).colorOverlay;
	...
	const redWeight = params?.redWeight ?? 1.0;
	const yellowWeight = params?.yellowWeight ?? 0.9;
		...
		<div style={{ position: "absolute", left: 12, top: 12, background: "rgba(0,0,0,0.75)", padding: 12, borderRadius: 8, color: "#fff", fontFamily: "sans-serif", zIndex: 1100, width: 360 }}>
			<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
				...
				<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
					...
					<select value={inputMode} onChange={(e) => { const mode = e.target.value as "Lw" | "Lp"; setParams(p => ({ ...p, inputMode: mode })); }} style={{ padding: "4px 6px", borderRadius: 4 }}>
			...
			{building.LwBySegment && Array.isArray(building.LwBySegment) && building.LwBySegment.map((entry: LwSide, idx: number) => {
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
				...
				<input type="range" min={1} max={60} step={0.5} value={currentYellowMax}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, yellowMaxDist: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
				...
				<input type="range" min={0} max={20} step={1} value={currentBlur}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, overlaySmoothSize: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
			...
			<div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
				<div style={{ flex: 1 }}>
					...
					<input type="range" min={0} max={2.0} step={0.05} value={yellowWeight}
						onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, yellowWeight: v })); setRefreshKey(k => k + 1); }} style={{ width: "100%" }} />
				...
				<div style={{ flex: 1 }}>
					...
					<input type="range" min={0} max={2.0} step={0.05} value={redWeight}
						onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, redWeight: v })); setRefreshKey(k => k + 1); }} style={{ width: "100%" }} />
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
				...
				<input type="range" min={40} max={100} step={1} value={currentRedThresh}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, redThreshold: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
				...
				<input type="range" min={20} max={90} step={1} value={currentYellowThresh}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, yellowThreshold: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
				...
				<input type="range" min={0} max={80} step={1} value={currentGreenThresh}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, greenThreshold: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
			...
			<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
				...
				<input type="range" min={0} max={60} step={1} value={currentBlueThresh}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = Number(e.target.value || 0); setParams(p => ({ ...p, colorOverlay: { ...p.colorOverlay, blueThreshold: v } })); setRefreshKey(k => k + 1); }} style={{ flex: 1 }} />
				...
				<button onClick={async () => {
					try {
						...
						if (Array.isArray(j?.segments)) {
							...
							const FOUR_PI_CONST = 10 * Math.log10(4 * Math.PI);
							const dbPerMeterLocal = params?.dbPerMeter ?? 0.5;
							const segs = j.segments.map((s: any) => {
								const v = Number(s?.value ?? 0);
								if (mode === "lp") {
									const lw = v + FOUR_PI_CONST + dbPerMeterLocal * 1.0;
									return { value: Number(lw) };
								}
								return { value: Number(v) };
							});
							while (segs.length < 4) segs.push({ value: 0 });
							setBuilding(b => ({ ...b, LwBySegment: segs.slice(0, 4) }));
							setRefreshKey(k => k + 1);
						}
